...
export default function Tasks() {
  ...
  const assignTaskToUser = (task) => {
    axiosClient
      .get("/users")
      .then(({ data }) => {
        const as400Users = data.data.filter((user) => user.user_type === "AS400");
      ...
      {assigningTask && (
        <div className="modal">
          <div className="modal-content">
            ...
            <select onChange={(e) => handleAssignAs400User(e.target.value)}>
            ...
            <select onChange={(e) => handleAssignWebUser(e.target.value)}>
            ...
            <button className="cancel_button" onClick={() => setAssigningTask(null)}>Cancel</button>
          </div>
        </div>
      )}
    </div>
